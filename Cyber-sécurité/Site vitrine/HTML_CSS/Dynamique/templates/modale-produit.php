<?php /** @var array $p */ ?>
<!-- Modale pour le produit -->
<div id="modal-<?php echo (int) $p['id']; ?>" class="modal" role="dialog" aria-modal="true"
    aria-labelledby="modalTitle-<?php echo (int) $p['id']; ?>">
    ...
    <div class="modal-card">
        <div class="flex items-start justify-between gap-3 border-b border-slate-200 p-5">
            <div>
                ...
                <h3 id="modalTitle-<?php echo (int) $p['id']; ?>" class="mt-1 text-lg font-extrabold text-slate-900">
        ...
        <div class="grid gap-5 p-5 md:grid-cols-2">
            <div class="rounded-3xl bg-gradient-to-br from-slate-900 to-emerald-900 p-5 text-white">
                ...
                <div class="mt-2 flex items-baseline justify-between">
                    <div class="text-3xl font-extrabold"><?php echo format_price($p['prix']); ?></div>
                    <div class="text-xs font-bold text-white/70"><?php echo (int) $p['stock']; ?> dispo.</div>
                </div>
                ...
                <?php if ((int) $p['stock'] > 0): ?>
                    <form method="post" action="/cart.php" class="mt-5">
                        ...
                        <input type="hidden" name="return_to" value="/index.php#modal-<?php echo (int) $p['id']; ?>">
                        <button type="submit"
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="aspect-video w-full overflow-hidden rounded-2xl border border-slate-200">
                    <img src="<?php echo htmlspecialchars('/' . ltrim((string) $p['image'], '/')); ?>"
                        alt="<?php echo htmlspecialchars($p['nom']); ?>" class="h-full w-full object-cover">
                </div>
